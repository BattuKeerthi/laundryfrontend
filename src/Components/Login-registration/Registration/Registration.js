...
const Registration = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post(`http://localhost:8090/laundry/register`, {
                name: data.name,
                email: data.email,
                phone: data.phone,
                state: data.state,
            ...
            if (error.response) {
        console.error("Server Error:", error.response.data);       // Backend error message
        console.error("Status Code:", error.response.status);      // HTTP error code
